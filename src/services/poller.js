...
module.exports = {
  start() {
    ...
    // Loop 1: Initial/deploy scan loop (runs once at startup, seeds and scans)
    (async () => {
      try {
        logger.info('poller: starting initialScan (Loop 1)');
    ...
    // Loop 2 (5mboundaryscan) and Loop 3 (newrootcandle open scan) running forever at precise 5m boundaries
    ...
    try {
      ...
      if (newSignals.length > 0) {
        ...
        if (notifyNewSignals) {
          ...
          logger.info('scanOnce: notifications suppressed for this run');
        }
      } else {
  ...
  scheduleAlignedTo5m() {
    ...
    const schedule = async () => {
      ...
      setTimeout(async () => {
        try {
          // Loop 2: 5mboundaryscan (precise scanning of all symbols for new root signals, seeding, macd check, and sending telegram per single signal block format)
          logger.info('poller: starting 5mboundaryscan (Loop 2)');
          await this.scanOnce({ notifyNewSignals: true });

          // MTF alignment/confirmation alerts from monitored root signals
          try {
            if (signalManager && typeof signalManager.checkMtfAlignment === 'function') {
              await signalManager.checkMtfAlignment();
            } else if (signalManager && typeof signalManager.checkMonitoredSignals === 'function') {
              await signalManager.checkMonitoredSignals();
            }
          } catch (e) {
            logger.debug({ e }, '5mboundaryscan: MTF alignment check failed');
          }
          ...
          // Loop 3: newrootcandle open scan (seeding klines, macd check, and triggering new root candle notifications)
          if (newRootTfs.length && config.NEW_ROOT_CANDLE_NOTIFY) {
            logger.info({ newRootTfs }, 'poller: starting newrootcandle open scan (Loop 3)');
            try {
