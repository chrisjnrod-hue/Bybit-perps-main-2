  ...
  try {
    ...
    while (true) {
      ...
      if (instruments.length > 0) {
        ...
        if (pageNum === 1 && instruments.length > 0) {
          const sampleSymbols = instruments.slice(0, 5).map(i => i.symbol);
          logger.debug({ sampleSymbols }, 'fetchAllSymbols: sample raw symbols from API (first page)');
        }
      } else {
    ...
    if (allSymbols.length === 0) {
      logger.warn(
        ...
        'fetchAllSymbols: no USDT.P symbols matched filter after full pagination. Attempting CoinGecko fallback.'
      );
      const fallback = await fetchSymbolsFromCoinGecko();
      if (fallback.length > 0) {
        logger.info({ count: fallback.length }, 'fetchAllSymbols: using CoinGecko fallback symbols');
        return fallback;
      }
      return [];
    }
