    ...
    try {
      ...
      if (config.MIN_MARKET_CAP > 0) {
        if (!mdata.market_cap || Number(mdata.market_cap) < config.MIN_MARKET_CAP) {
          logger.info({ symbol, market_cap: mdata.market_cap }, 'Filtered out by MIN_MARKET_CAP');
          return;
        }
      }
      ...
      if (config.MIN_24H_USDT_VOLUME > 0) {
        if (!mdata.volume_24h_usdt || Number(mdata.volume_24h_usdt) < config.MIN_24H_USDT_VOLUME) {
          logger.info({ symbol, volume_24h_usdt: mdata.volume_24h_usdt }, 'Filtered out by MIN_24H_USDT_VOLUME');
          return;
        }
      }
      ...
      if (accept && accept.decision === 'accept' && config.OPENTRADE) {
